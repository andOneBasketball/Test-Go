...
func DeCompressZip(zipFile, dest, passwd string, offset int64) ([]string, error) {
	...
	for _, f := range zr.File {
		...
		subFilenames = append(subFilenames, fpath)
	}
...
func main() {
	startTime := time.Now()
	subFilenames, err := DeCompressZip("./test.zip", "./tmp", "abcd1234", 0)
	if err != nil {
		fmt.Println(err)
	}
	fmt.Printf("decompress zip file %f\n", time.Now().Sub(startTime).Seconds())
	fmt.Printf("filename is %v", subFilenames)
