...
/*
@brief: 解压zip包
@param[in]: zipFile 待解压数据包的全路径
@param[in]: dest 将数据包解压到指定目录的目录全路径
@param[in]: passwd 待解压数据包的密码
@param[in]: offset 待解压数据包的偏移量
@return 成功返回zip包的文件列表,失败返回具体错误
*/
func DecompressZip(zipFile, dest, passwd string, offset int64) ([]string, error) {
	...
	for _, f := range zr.File {
		...
		subFilenames = append(subFilenames, f.Name)
	}
